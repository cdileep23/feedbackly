...
export const updateFeedbackForm = async (req, res) => {
  try {
    ...
    const existingForm = await FeedbackModel.findById(formId);
    if (!existingForm) {
      return res.status(404).json({
        success: false,
        message: "Feedback form not found",
    ...
    if (existingForm.isActive) {
      return res.status(400).json({
        success: false,
        message: "Cannot update active feedback form. Please close it first.",
      });
    }
    ...
    // Clean and prepare questions if provided
    ...
    // Update the form
    const updatedForm = await FeedbackModel.findByIdAndUpdate(
      formId,
      updateData,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        questionsCount: updatedForm.questions.length,
        isActive: updatedForm.isActive,
